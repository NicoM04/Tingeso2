...
const checkIncomeToPaymentRatio = (data, monthlyIncome) => {
    return httpClient.post(`/api/evaluation/check-income-to-payment-ratio?monthlyIncome=${monthlyIncome}`, data);
};

const checkCreditHistory = (hasGoodCreditHistory) => {
    return httpClient.post(`/api/evaluation/check-credit-history?hasGoodCreditHistory=${hasGoodCreditHistory}`);
};
...
const checkEmploymentStability = (employmentYears, isSelfEmployed, incomeYears) => {
    return httpClient.post(`/api/evaluation/check-employment-stability?employmentYears=${employmentYears}&isSelfEmployed=${isSelfEmployed}&incomeYears=${incomeYears}`);
};


// Verificar relación deuda/ingreso
const checkDebtToIncomeRatio = (data, totalDebt, monthlyIncome) => {
    return httpClient.post(`/api/evaluation/check-debt-to-income-ratio?totalDebt=${totalDebt}&monthlyIncome=${monthlyIncome}`, data);
};

// Verificar monto máximo financiable
const checkMaximumLoanAmount = (data, propertyValue) => {
    return httpClient.post(`/api/evaluation/check-maximum-loan-amount?propertyValue=${propertyValue}`, data);
};


// Verificar si el préstamo puede finalizar antes de los 75 años
const checkApplicantAge = (applicantAge, credit) => {
    return httpClient.post(`/api/evaluation/check-applicant-age?applicantAge=${applicantAge}`, credit);
};
...
const checkSavingsCapacity = (hasMinimumBalance, hasConsistentSavings, hasRegularDeposits, meetsBalanceYears, hasNoRecentWithdrawals) => {
    return httpClient.post(`/api/evaluation/evaluate-savings-capacity?hasMinimumBalance=${hasMinimumBalance}&hasConsistentSavings=${hasConsistentSavings}&hasRegularDeposits=${hasRegularDeposits}&meetsBalanceYears=${meetsBalanceYears}&hasNoRecentWithdrawals=${hasNoRecentWithdrawals}`);
};
...
// Obtener todos los créditos de un cliente por ID (lista)
